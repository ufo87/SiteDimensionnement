...
function initV3V(debit, pdc_ajoutes, tableau)
{
    debit /= 1000;
    for (var i=0; i < tableau.length; i++)
    {
        tableau[i][4] = Math.pow(debit / parseFloat(tableau[i][3]), 2) * 100;
        tableau[i][5] = Math.round((parseFloat(tableau[i][4]) / (parseFloat(tableau[i][4]) + parseFloat(pdc_ajoutes)))*100)/100;
...
function recuperationInformation()
{
  ...
  _debitD2 = document.getElementById("debitEauMelange").value;
  _debitD2 = parseFloat(_debitD2);
	_tempT1 = document.getElementById("temperatureEau1").value;
  _tempT1 = parseFloat(_tempT1);
	_tempT2 = document.getElementById("temperatureEau2").value;
  _tempT2 = parseFloat(_tempT2);
  _tempT3 = document.getElementById("temperatureEau3").value;
  _tempT3 = parseFloat(_tempT3);
  _tauxGlycol = document.getElementById("tauxGlycolMelange").value;
  _tauxGlycol = parseFloat(_tauxGlycol);
  _pdc_ajoutes = document.getElementById("pdc_ajouteMelange").value;
  _pdc_ajoutes = parseFloat(_pdc_ajoutes);

  save();

	if (checkValidity(_tempT1, _tempT2, _tempT3, _tauxGlycol, _debitD2)=="")
  {
      resultat = calculMelange(_tempT1, _tempT2, _tempT3, _tauxGlycol, _debitD2);
      if (resultat.length==0)
      {
          document.getElementById('erreur_tableau_vide').innerHTML = "Valeurs non cohérentes";
      }
      else
      {

          document.getElementById('erreur_tableau_vide').innerHTML = "Débit D3 = " + round(resultat[1],2) + " l/h" + "<br>";
          document.getElementById('erreur_tableau_vide').innerHTML += "Débit D1 = " + round(resultat[0],2) + " l/h";

          var tableau1 = [];
          tableau1.push(calculDN("Débit D1", resultat[0], 30, _tauxGlycol, _tempT1));
          tableau1.push(calculDN("Débit D2", _debitD2, 30, _tauxGlycol, _tempT2));
          tableau1.push(calculDN("Débit D3", resultat[1], 30, _tauxGlycol, _tempT3));
          generate_table(tableau1, "tableauDN", ["PDC [daPa/m]", "Vitesse [m/s]", "Diamètre","Débit", "Nom"]);

          if (checkValidityPdC(_pdc_ajoutes)=="")
          {
              resultatV3V = initV3V(_debitD2, _pdc_ajoutes, tableauV3V_spherique);
              ...
              var cible = round((0.01 * resultat[0] / Math.pow(_pdc_ajoutes, 0.5)), 2);
              var tableauVanneTA = [];
  ...
  else
  {
      ...
      document.getElementById('erreur_tableau_vide').innerHTML = checkValidity(_tempT1, _tempT2, _tempT3, _tauxGlycol, _debitD2);
  }
}
...
function checkValidity(tempT1, tempT2, tempT3, tauxGlycol, debit)
{
    if (debit==='none' || debit==="" || isNaN(debit))
    ...
    if (tempT1===null || tempT1==='none' || tempT1==="" || isNaN(tempT1))
        return "La température T1 est mal renseignée";
    if (tempT1>85)
        return "La température T1 est trop élevée";
    if (tempT1<-50)
        return "La température T1 est trop faible";

    if (tempT2===null || tempT2==='none' || tempT2==="" || isNaN(tempT2))
        return "La température T2 est mal renseignée";
    if (tempT2>85)
        return "La température T2 est trop élevée";
    if (tempT2<-50)
        return "La température T2 est trop faible";

    if (tempT3===null || tempT3==='none' || tempT3==="" || isNaN(tempT3))
        return "La température T3 est mal renseignée";
    if (tempT3>85)
        return "La température T3 est trop élevée";
    if (tempT3<-50)
        return "La température T3 est trop faible";

    if (tauxGlycol===null || tauxGlycol==='none' || tauxGlycol==="" || isNaN(tauxGlycol))
        return "Le taux de glycol est mal renseigné";
    if (tauxGlycol>50)
        return "Le taux de glycol est trop élevé";
    if (tauxGlycol<0)
...
function calculViscosite(pourcentageGlycol, tempEau)
{
    ...
    if (mu<0)
        return "erreur pour le calcul de la viscosité";
    return mu;
}
...
function calculMelange(tempT1, tempT2, tempT3, tauxGlycol, debitD2)
{
    var debitD3 = 0.0;
    var debitD1 = 0.0;

    debitD3 = debitD2*(tempT2-tempT1) / (tempT3-tempT1);
    debitD1 = debitD2 - debitD3;

    if (debitD1 <=0 || debitD3 <= 0) {
      return [];
    }

    return [debitD1, debitD3];
}

function calculDN(name, debit, pdcVise, tauxGlycol, tempEau)
{
    var diamNominal = 0.0;
    var viscosite = 0.0;
    ...
    tableauResultat.push(name);
    tableauResultat.push(round(debit, 2));
    tableauResultat.push(listeDN[listeDiamInt.indexOf(diamNominal)]);
    tableauResultat.push(Math.round(calculVitesse(debit, listeDiamInt[listeDiamInt.indexOf(diamNominal)])*100)/100);
    tableauResultat.push(Math.round(calculPdC(materiau, debit, listeDiamInt[listeDiamInt.indexOf(diamNominal)], tauxGlycol, tempEau)*100)/100);
...
function save()
{
	localStorage.setItem('debitEauMelange', document.getElementById("debitEauMelange").value);
	localStorage.setItem('temperatureEau1', document.getElementById("temperatureEau1").value);
	localStorage.setItem('tauxGlycolMelange', document.getElementById("tauxGlycolMelange").value);
	localStorage.setItem('temperatureEau2', document.getElementById("temperatureEau2").value);
	localStorage.setItem('temperatureEau3', document.getElementById("temperatureEau3").value);
	localStorage.setItem('pdc_ajouteMelange', document.getElementById("pdc_ajouteMelange").value);
}

function load()
{
  	if(localStorage.hasOwnProperty("temperatureEau1"))
  		document.getElementById("temperatureEau1").value = localStorage.getItem("temperatureEau1");
  	else
  		document.getElementById("temperatureEau1").value = 10;

  	if(localStorage.hasOwnProperty("temperatureEau2"))
  		document.getElementById("temperatureEau2").value = localStorage.getItem("temperatureEau2");
  	else
  		document.getElementById("temperatureEau2").value = 35;

  	if(localStorage.hasOwnProperty("tauxGlycolMelange"))
  		document.getElementById("tauxGlycolMelange").value = localStorage.getItem("tauxGlycolMelange");
  	else
  		document.getElementById("tauxGlycolMelange").value = 0;

    if(localStorage.hasOwnProperty("temperatureEau3"))
  		document.getElementById("temperatureEau3").value = localStorage.getItem("temperatureEau3");
  	else
  		document.getElementById("temperatureEau3").value = 50;

    if(localStorage.hasOwnProperty("debitEauMelange"))
      document.getElementById("debitEauMelange").value = localStorage.getItem("debitEauMelange");
    else
      document.getElementById("debitEauMelange").value = 1000;

    if(localStorage.hasOwnProperty("pdc_ajouteMelange"))
      document.getElementById("pdc_ajouteMelange").value = localStorage.getItem("pdc_ajouteMelange");
    else
      document.getElementById("pdc_ajouteMelange").value = 20;
